...
<Playground>
  {() => {
    ...
    return (
      <div>
        <button
          disabled={localMarkers.length === markers.length}
          onClick={() =>
            setLocalMarkers([...localMarkers, markers[localMarkers.length]])
          }
        >
          Add Marker
        </button>
        <button
          disabled={localMarkers.length === 0}
          onClick={() => setLocalMarkers(localMarkers.slice(1))}
        >
          Remove Marker
        </button>
        <ReactGlobe markers={localMarkers} />
      </div>
    );
  }}
</Playground>
