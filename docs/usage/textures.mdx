...
You can use the `onGlobeTextureLoaded`, `onGlobeBackgroundTextureLoaded` and `onGlobeCloudsTextureLoaded` callback to capture the event after the texture has successfully been applied.

<Playground>
  {() => {
    const globeBackgroundTexture = 'https://raw.githubusercontent.com/chrisrzhou/react-globe/main/textures/background.jpg';
