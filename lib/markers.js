...
export function updateMarkerObjects(
  markerObjects,
  { callbacks, markers, options, tooltip },
) {
  const {
  ...
  markerObjects.children.forEach(markerObject => {
    ...
    applyCallbacks(markerObject, { callbacks, options, tooltip });
  });
}
...
function applyCallbacks(markerObject, { callbacks, options, tooltip }) {
  const { enableMarkerTooltip, markerTooltipRenderer } = options;
  ...
  markerObject.on('mousemove', interaction => {
    const event = interaction.data.originalEvent;
    if (enableMarkerTooltip) {
      tooltip.show(
        event.clientX,
        event.clientY,
        markerTooltipRenderer(markerObject.marker),
      );
    }
